...
2. **The accuracy-floor REFUSAL gate in `compression.py` is a feature, never soften it.** If a compressed artifact can't hold `max(accuracy_floor_relative * baseline_accuracy, constraints.accuracy_floor)`, the factory refuses to ship it and keeps the uncompressed one. Never make the gate "try harder" to pass, never bypass it, never silently ship a failing artifact.
